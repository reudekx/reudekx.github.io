...
* WITH절을 이용한 재귀 쿼리
    * 재귀 부분의 각 반복은 이전 반복에서 생성된 행들에 대해서만 작동한다는 점을 인지하자.
...
또한 처음에는 조건을 모두 ON절에 적을지 고민을 하였지만,
검색해 본 결과 웬만하면 옵티마이저가 최적화를 수행하여 차이가 없다는 정보를 찾게 되어
두 테이블을 연결하는 핵심 조건만 ON절에 적고 나머지는 WHERE절에 적는 것으로 쿼리를 작성하였다.
