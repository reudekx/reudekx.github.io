...
* **xvalue**
    ...
    * **이동될 수 있다.**
        ...
        * 혹은 이동 생성자 및 이동 대입 연산자가 삭제된 경우도..
...
앞선 코드를 통해 표현식의 값 범주를 확인해 볼 수 있었다.
이번엔 다음과 같은 코드를 확인해 보자.
...
```
위 코드를 x86-64 gcc 14.2로 컴파일하면 (-std=c++17 -O0 옵션 사용)
대략 다음과 같은 어셈블리 코드가 생성된다. (main 함수만 나타내었음.)
...
main:
    ...
    ret
```
9번째 줄을 보면, `Object{2}`라는 **prvalue** 표현식이 임시 객체로 구체화되어 스택 공간을 점유하게 된 것을 확인할 수 있다.
뒤이어 10번째와 11번째 줄에서는 해당 임시 객체의 주소가 this 포인터로서 함수에 전달되는데,
이때 임시 객체를 **xvalue**로 취급하여 && 레퍼런스 한정자가 지정된 `get_value` 함수를 호출한 것을 볼 수 있다.

[레퍼런스 문서](https://en.cppreference.com/w/cpp/language/implicit_conversion)에서는 다음과 같이 서술하고 있다.
...
* [https://godbolt.org/](https://godbolt.org/) - C/C++ 코드를 어셈블리로 컴파일한 결과 확인 가능
